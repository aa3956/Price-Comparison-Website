...
public class Lamborghini extends WebScraper {
    ...
    public void run(){
        ...
        while(runThread){
            ...
            for (int i = 1; i < 5; i++) {
                ...
                webUrl = "https://www.hrowen.co.uk/lamborghini/page-" + i + "/used-lamborghini/default/-1/-1.aspx";
                System.out.println("This is the foor looop; " + webUrl);
                
                System.out.println("Lamborghini thread is scraping data.");

                Document document5 = null;
                try {
                       document5 = Jsoup.connect(webUrl).timeout(6000).get();

                } catch (IOException ex) {
                      Logger.getLogger(Lamborghini.class.getName()).log(Level.SEVERE, null, ex);
                    }

                //Grab the div that wraps the data
                Elements ele = document5.select("div.grid");
